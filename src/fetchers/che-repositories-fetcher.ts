...
export class Repositories {
  name: string;
  url: string;
  useCheMilestone: boolean;
}

@injectable()
export class CheRepositoriesFetcher {
  public static readonly CHE_REPOSITORIES_YAML = 'https://raw.githubusercontent.com/benoitf/che/component-repositories/.repositories.yaml';
  ...
  async init(): Promise<Repositories[]> {
    ...
    if (parsedRepositories) {
      ...
      parsedRepositories.forEach((repoDef: any) => {
        repositories.push({ name: repoDef.name, url: repoDef.url, useCheMilestone: repoDef.useCheMilestone });
      });
    } else {
